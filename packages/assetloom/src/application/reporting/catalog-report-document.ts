...
</style></head><body><main><header class="hero"><small>Configurable resource catalog</small><h1>${escapeHtml(model.configurationName)}</h1><p>Deterministic inventory for ${escapeHtml(model.targets.join(', ') || 'no selected targets')}</p><div class="metrics"><div><strong>${model.resources.length}</strong><span>Resources</span></div><div><strong>${model.outputs}</strong><span>Outputs</span></div><div><strong>${model.issues}</strong><span>Issues</span></div></div></header>${sections}<details><summary>Effective configuration</summary><pre><code>${escapeHtml(model.effectiveConfiguration)}</code></pre></details></main></body></html>\n`;
}
