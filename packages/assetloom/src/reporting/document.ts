...
export function renderReportDocument(model: ReportModel): string {
  ...
  const integration =
    model.integration.outputs.length === 0
      ? ''
      : `
        <section class="overview-panel integration" id="native-integration">
  ...
  <div class="layout">
    ...
    <main class="main">
      <header class="hero">
        <div class="hero-top">
          <div>
            <p class="overline">Native asset quality report</p>
            <h1>${escapeHtml(model.configurationName)}</h1>
            <p class="hero-description">${escapeHtml(model.description ?? 'Source artwork and generated Android and iOS resources in one reviewable artifact.')}</p>
          </div>
        ...
        <div class="metrics">
          ...
          <div class="metric"><strong>${model.outputs}</strong><span>Native outputs inspected</span></div>
          <div class="metric"><strong>${model.validOutputs}/${model.outputs}</strong><span>Outputs verified</span></div>
        </div>
      </header>
      <div class="toolbar">
        <span class="toolbar-label">Filter the report by native platform</span>
        ...
        <section class="overview-panel" id="overview">
          <div class="overview-grid">
            <div>
              ...
              <p class="overview-copy">The previews below embed the actual configured inputs and the native files present when this report was created. The report has no network or external-file dependencies and can be copied as a single HTML file.</p>
      ...
      <footer class="footer">Generated by Assetloom · Android and iOS native resources · Self-contained report</footer>
    </main>
  </div>
